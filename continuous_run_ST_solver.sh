...
check_celery_and_execute () {
    ...
    then
        ...
        then
            ...
            (export PYTHONPATH="$ST_INSTALL_PATH/src/:$PYTHONPATH" && nohup env $(cat $ST_INSTALL_PATH/.env | xargs) celery -A worker_short_term_solver.app worker -Q short_term_solver --concurrency=5 -P prefork -n short_term_solver_worker@%h --loglevel=debug > solver.out 2>&1 &)
        elif [ "$MODEL_MODE" = "ST_SCHEDULER" ]
        then
            python3 -m pip install -r $ST_INSTALL_PATH/requirements.txt > /dev/null
            (export PYTHONPATH="$ST_INSTALL_PATH/src/:$PYTHONPATH" && nohup env $(cat $ST_INSTALL_PATH/.env | xargs) celery -A worker_short_term_scheduler.app worker -Q short_term_scheduler --concurrency=3 -P prefork -n short_term_scheduler_worker@%h --loglevel=debug > scheduler.out 2>&1 &)
        fi
    fi
}
...
while true
do
  export $(grep -v '^#' .env_solver | sed 's/~/$HOME/' | envsubst | xargs -0)
